# Codex / AI Agent 产品资料模板转换工具 V3

简体中文 | [English](README.en.md)

**面向 Codex 等 AI Agent 的文档转换工具：把不同格式的产品资料，整理成统一公司模板的产品介绍文档。**
...
## 与 Codex 等 AI Agent 配合使用

本项目围绕 Agent 辅助的文档处理流程设计。由 Codex 或其他能够读取本地文件、执行命令并处理结构化 JSON 的 AI Agent，调用 Python 脚本、整理内容决策和解释检查结果；用户负责确认业务内容与模板复核。也可以由熟悉流程的人员直接运行命令。

典型协作流程：

1. 用户向 Agent 提供源资料、目标模板和交付目标（DOCX 或 PDF）。
2. Agent 按下文安装要求检查环境，调用 `run_pipeline.py` 提取内容并生成任务包。
3. Agent 依据原文和任务包整理 `ContentDecisionV3`；不确定的内容交给用户确认。
4. 用户在生成的复核材料中完成确认，取得有效回执；Agent 携带决策文件和回执继续运行。
5. Agent 检查结果，仅在 `status=PASS` 且 `deliverable=true` 时交付文件。

Agent 负责调用和内容辅助，Python 脚本负责转换与校验。项目不内置 Agent 客户端、模型服务或 API Key，也不是已经封装好的 MCP 服务。使用其他 Agent 时，需要自行提供相应的本地文件与命令执行能力。
...
- **3sssm0keee**：项目维护者，负责需求、业务复核与发布决策。
- **Codex（OpenAI AI 编程 Agent）**：AI 开发协作者，参与代码实现、问题排查、验证辅助和中英文文档编写；业务批准与发布决策由项目维护者负责。
